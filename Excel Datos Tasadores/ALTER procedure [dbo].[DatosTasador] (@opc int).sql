...
AS
  ...
  DROP TABLE #tablaPruebas; 
	declare @tabla table(Activo varchar(2),EnPrueba varchar(2),Visitador varchar(2),TipoEncargo varchar(200),TasadorATV varchar(20),
...
BorsanUnico AS (
  ...
  ORDER BY 
	...
	WHEN codprofe = '29' THEN 5
	WHEN codprofe = '30' THEN 6
       ELSE 7
     END
) AS rn
   ...
   --     --    WHEN ISNULL(objet.objetos, '') in ('Gestión (80011),','')           THEN 'No'
	...
	--	WHEN isnull(borsan.codprofe,'') in ('25','27','28') then ''
		...
		as EnPrueba,
		case
			 when isnull (borsan.codprofe,'') = '25' and EXISTS (SELECT 1 FROM CORITEL..taortapf p WHERE p.codtasad = a.codtasad AND p.codprofe = '30') then 'SOLO TH BORSAN + ATVALOR'
			 when isnull (borsan.codprofe,'') = '25' then 'SOLO TH BORSAN'	----requiere datos bde 
			 when isnull (borsan.codprofe,'') = '26' and EXISTS (SELECT 1 FROM CORITEL..taortapf p WHERE p.codtasad = a.codtasad AND p.codprofe = '30') then 'TH + TH BORSAN + ATVALOR'
			 when isnull (borsan.codprofe,'') = '26' then 'TH + TH BORSAN'	----requiere datos bde 
			 when isnull (borsan.codprofe,'') = '27' and EXISTS (SELECT 1 FROM CORITEL..taortapf p WHERE p.codtasad = a.codtasad AND p.codprofe = '30') then 'VISITADOR TH + ATVALOR'
			 when isnull (borsan.codprofe,'') = '27' then 'VISITADOR TH'	----requiere datos bde 
			 when isnull (borsan.codprofe,'') = '28' and EXISTS (SELECT 1 FROM CORITEL..taortapf p WHERE p.codtasad = a.codtasad AND p.codprofe = '30') then 'CONSULTORIA + ATVALOR'
			 when isnull (borsan.codprofe,'') = '28' then 'CONSULTORIA'		----requiere datos bde 
			 when isnull (borsan.codprofe,'') = '18' and EXISTS (SELECT 1 FROM CORITEL..taortapf p WHERE p.codtasad = a.codtasad AND p.codprofe = '30') then 'VISITADOR EXTERNO + ATVALOR'
			 when isnull (borsan.codprofe,'') = '18' then 'VISITADOR EXTERNO'
			 when isnull (borsan.codprofe,'') = '19' and EXISTS (SELECT 1 FROM CORITEL..taortapf p WHERE p.codtasad = a.codtasad AND p.codprofe = '30') then 'VISITADOR EXTERNO + ATVALOR'
			 when isnull (borsan.codprofe,'') = '19' then 'VISITADOR EXTERNO'
			 when EXISTS (SELECT 1 FROM CORITEL..taortapf p WHERE p.codtasad = a.codtasad AND p.codprofe = '29') 
			      AND EXISTS (SELECT 1 FROM CORITEL..taortapf p WHERE p.codtasad = a.codtasad AND p.codprofe = '30') then 'TH + ATVALOR'
