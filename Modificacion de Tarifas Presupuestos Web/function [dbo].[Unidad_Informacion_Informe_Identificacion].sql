...
GO
ALTER function [dbo].[Unidad_Informacion_Informe_Identificacion]
                ( @codigo   int 
																 ,@usuario  int 
																 ,@reducido bit 
																 ,@ancho    varchar(100))
...
as
   ...
   declare @direccion_informe             varchar(1000)
          ...
          ,@fk_TH_Entidades_SLA           int
          ,@SLA_dias_naturales            int
          ,@SLA_dias_laborables           int
          ...
          ,@dias_quedan                   decimal(19,15)
          ,@FIRMA_TASADOR                 varchar(max)
		 ...
		 declare @pdffinalcliente bit
          ...
          ,@historial_clonacion varchar(max)
		
		declare @codigoAutofacturacion  int
	 declare @EstadoAutofacturacion  varchar(200)
	 declare @lista_PE varchar(max)	
		
  declare @span_boton varchar(max)=
   'vertical-align:middle;font-size:0.65vw;padding:2px;font-weight:bold;border-radius:4px;-ms-border-radius:4px;-moz-border-radius:4px;-webkit-border-radius:4px;-khtml-border-radius:4px'
  +';box-shadow:0px 2px 2px 0px rgba(0,0,0,0.3)'        /* CSS3 */
  +';-webkit-box-shadow:0px 2px 2px 0px rgba(0,0,0,0.3)' /* Safari and Chrome */
  +';-moz-box-shadow:0px 2px 2px 0px rgba(0,0,0,0.3)' /* Firefox */
  +';-ms-box-shadow:0px 2px 2px 0px rgba(0,0,0,0.3)' /* Internet Explorer */
  +';-o-box-shadow :0px 2px 2px 0px rgba(0,0,0,0.3)' /* Opera */
  +';'
  ...
  if isnull(@usuario,0)!=0
         ...
         select @numinfor            =isnull(n.numinfor,'')
               ...
               ,@fk_TH_Entidades_SLA =a.fk_TH_Entidades_SLA 
               ,@SLA_dias_naturales  =a.SLA_dias_naturales  
               ,@SLA_dias_laborables =a.SLA_dias_laborables 
               ...
               ,@historial_clonacion = case when (select count(*) from CORITEL.dbo.taocodin c (nolock) where c.numinfor=n.numinfor and c.codincid in ('CLO','CLS','CLX'))=0 then ''
                                            else '<hr style="width:100%;border:solid 0px;border-top:solid 1px transparent;padding:0px;">'
                                                 +'<div id="historia_clonacion_'+@numinfor+'" sytle="display:inline-block;width=100%">'
                                                 +case when n.fecha_vision_historial_en_grid is null or (n.fecha_vision_historial_en_grid is not null and n.fecha_vision_historial_en_grid<dateadd(hour,-8,getdate()))
                                                       then '<span style="cursor:pointer;'+@span_boton+';background:gold" onclick="WSQL(''TH_Informes_Mostrar_Historial_Clonacion @numinfor=·'+@numinfor+'·, @usuario='+format(@usuario,'0')+' '')">Mostrar Historial Clonación</span>'
                                                       else isnull(dbo.TH_Informes_Tabla_Historia_Clonacion (n.numinfor, @usuario),'')
                                                           +'<br><span style="cursor:pointer;'+@span_boton+';background:gold" onclick="WSQL(''TH_Informes_Ocultar_Historial_Clonacion @numinfor=·'+@numinfor+'·, @usuario='+format(@usuario,'0')+' '')">Ocultar Historial Clonación</span>'
                                                  end
                                                 +'</div>'  
                                                 +'<hr style="width:100%;border:solid 0px;border-top:solid 1px transparent;padding:0px;">'
                                        end
               ,@FIRMA_TASADOR       =dbo.FIRMA_TASADOR (isnull(n.numinfor,'XXXXXXXXX'))
          ...
          set @dias_quedan=dbo.f_Horas_Laborables(getdate(),@fecha_limite_entrega,null,null,1,1,1)/24.00
          ...
          declare @bloqueado bit
                 ...
                 ,@usuario_con_permisos_asignar_otro_gestor bit
          ...
          if @estinfor in ('0')
             begin
                ...
                set @usuario_gestor_encargado=0; if @usuario_TH=1 and exists (select codigo from ETHER.dbo.usuarios u (nolock) where u.codigo = @usuario and u.cod_usuario_TH = (select top 1 codusuar from CORITEL.dbo.th_gestor_encargo_log where numinfor =@numinfor order by fecha_asignacion desc) ) set @usuario_gestor_encargado =1
                set @usuario_con_permisos_asignar_otro_gestor=0; if charindex('42', @lista_PE)>0 set @usuario_con_permisos_asignar_otro_gestor =1
             end
   ...
   -------------------------
      ...
      +'<hr style="width:100%;border:solid 0px;border-top:solid 1px transparent;padding:0px;">'						
             ...
             +case when 1=2 then '' else 
              ...
              -------------------------------------------------------------------------------- 
             +case when 1=2 then '' 
                   when @estinfor in ('X') then ''
                   when @estinfor in ('4','5','6') then ''
                   when @informe_de_pruebas =1 then ''
                   else case when @fecha_limite_entrega is not null 
                             then '<span style="display:inline-block;color:#999;font-size:0.6vw;padding-right:4px;vertical-align:middle;">Lím. Entrega:</span>'
										                       +isnull('<span class="resaltado_general" style="background:'+@color_limite_entrega+''+case when @fecha_limite_entrega_forzada=1 then ';border:solid 3px #a00;' else '' end+';vertical-align:middle;padding-right:3px;"'
                                          +' title="Fecha Límite de Entrega"'
				                                      +' onclick=" var  p='''';'
				                                                ...
				                                                +' Pide_Parametros(p,e);"'
                                          +'>'+format(@fecha_limite_entrega,'dd/MM/yy HH:mm')+'</span>', '')
                                 ---------------------------------------------------------         
                                 +'<span class="etiliment">'+isnull('('+ETHER.dbo.f_dia_semana(@fecha_limite_entrega)+')','')+'</span>'
                                 +'<span class="etiliment">'+isnull(', quedan: <font class="datliment" '+case when @dias_quedan<0 then 'style="background:red;color:white;padding:1px;"'  else 'style="padding:1px;background:green;color:white;"' end+'>'+format(@dias_quedan, '#,0.00','de-DE')+'</font> días','')+'</span>'
                                 +'<span class="etiliment">, Cód.SLA: '       +isnull('<font class="datliment">'+format(@fk_TH_Entidades_SLA,'#,0','de-DE')+'</font>','--')+'</span>'
                                 +'<span class="etiliment">, Días => Labor.: '+isnull('<font class="datliment">'+format(@SLA_dias_laborables,'#,0','de-DE')+'</font>','--')+'</span>'
                                 +'<span class="etiliment">, Natur.: '        +isnull('<font class="datliment">'+format(@SLA_dias_naturales ,'#,0','de-DE')+'</font>','--')+'</span>'
			                          else ''
		                      end
              end
						 end
     -- ===================================
		   -- Objeto + Finalidad + Estado
		   -- ===================================
     +'<br>'
     ...
     --------------------------------------------------
     -- Código incrustrado por Kenny
     --------------------------------------------------
     +case when @usuario_con_permisos_asignar_otro_gestor = 0 then '' else '<img class="aumenta click imginforme" style="padding-left:0.5vw;;vertical-align:middle" src="data:image/png;base64," '
                             +' title="Asignar encargo a otro gestor." '
				                         +' onclick="Pide_Parametros( '''
				                         +'{combo|Asignación Gestor al Informe '+@numinfor+'|Gestor||TH_taousuario|tipusuar = ··P·· and anulado = 0[style font-family:monospace;font-size:60%;width:500px;]|||}'
                             +' '',''WSQL(··TH_Informes_Asignar_Otro_Gestor ·'+@numinfor+'·, #parametro_value_1#'+' ··)'');">' end
     ---------------------------------------------------
     ---------------------------------------------------
     ...
     -- ======= Asignacion ===========
     -- ==============================
     +case when ((@estinfor in ('0') and @bloqueado=0) or (@estinfor in ('3'))) then
     ...
     -- ====== Asignacion por Mapa ======
     -- =================================
     +case when @estinfor in ('0') and @bloqueado=0 and n.latitud is not null 
     ...
     -- ======= Anulación  ===========
     -- ==============================
		    +case when ( @estinfor in ('0','1','2') ) or ( @estinfor in ('5') and exists (select * from dbo.TH_Informes_Historia_Clonacion (@numinfor)) )
            then '<img class="aumenta click imginforme" src="img\anular_encargo.png" '
		                  +' title="Anulación Manual del encargo"'
		                  +' onclick=" var p=''''; p+=''{confirmacion|Anular el Encargo '+@numinfor+'|||''; var e=''WSQL(··TH_Informer_Anulacion_Manual ·'+@numinfor+'·, @usuario='+format(@usuario,'0')+' ··)''; Pide_Parametros(p,e);"'
                +' >'
			         else ''
		     end
     -- ==================================================
     ...
     +isnull('<span style="display:inline-block;width:15vw;vertical-align:middle;color:#0aa;font-size:0.65vw;font-weight:bold;word-break:break-all;border:solid 1px transparent;">'
            +case when isnull(@fecemifa,'')!=''          then 'Facturado el <i>('+format(@fecemifa,'dd/MM/yy HH:mm')+')</i>'
                               ...
                               where h.numinfor=@numinfor 
					             ...
					             when isnull(n.importe_provision_en_banco,0)!=0 then 'Provisionado '+case when n.fecha_ingreso_provision is not null then ' => Ingresado en Banco' else '' end
     ...
     +case when @acc_provision=1 then
            ...
            +case when isnull(n.importe_provision_en_banco,0)!=0 then 
                      +isnull('<span style="padding-left:0.5vw;vertical-align:middle">Provisión: '+format(n.importe_provision_en_banco,'#,0.00 €','de-DE')+'</span>','')
                      +isnull('<span style="padding-left:0.0vw;vertical-align:middle;font-size:0.50vw;">, Pagado en Banco el : '+format(n.fecha_ingreso_provision,'dd/MM/yyyy HH:mm')+'</span>','')
                  else 
   ...
   +case when @estinfor in ('3','4','5','6','F') then      
    ...
    -- =============================
		      ...
		      +isnull( 'Gestor: ' + (select top 1 u.desusuar from (select top 1 * from CORITEL.dbo.th_gestor_encargo_log (nolock) where numinfor = @numinfor  order by fecha_asignacion desc ) ge inner join CORITEL.dbo.taousuar (nolock) u on ge.codusuar = u.codusuar )
              ,'Sin gestor asignado')     
			  +' </span>'
				 end
				 +case when 1=2 then '' else 
   ...
   +'<span style="display:inline-block;color:#1F618D;font-size:0.60vw;vertical-align:middle;width:18vw;border:solid 4px transparent">'
		       +case when @estinfor not in ('4', '5', '6', 'F') or e.fecsuper<convert(datetime,'15/08/2025',103) then ''
			            else '<b>F.Dig.&nbsp;</b>'
		             -- ==============================
               -- ======= Estado de la Firma ===
               -- ==============================
               +isnull('<span style="display:inline-block;width:auto;color:#070;font-size:120%;vertical-align:middle;">'+@FIRMA_TASADOR+'</span>','')
               +'<span style="display:inline-block;width:0.5vw"></span>'
               +case when @firmado_digital=1 and charindex('(6)',@FIRMA_TASADOR)>0 then '<span style="display:inline-block;width:auto;color:#000;font-size:100%;vertical-align:middle;"> >= el <font style="font-size:120%;color:#000;" title="Fecha Firma Digital Tasador">'+format(@fecha_firma_digital,'dd/MM/yyyy HH:mm:ss')+'</font></span>'
                     when @firmado_digital=1 and charindex('(4)',@FIRMA_TASADOR)>0 then '<span style="display:inline-block;width:auto;color:#000;font-size:100%;vertical-align:middle;"> >= el <font style="font-size:120%;color:#000;" title="Fecha Firma Digital Tasador">'+format(@fecha_firma_digital,'dd/MM/yyyy HH:mm:ss')+'</font></span>'
                     when @firmado_digital=1 and charindex('(3)',@FIRMA_TASADOR)>0 then '<span style="display:inline-block;width:auto;color:#000;font-size:100%;vertical-align:middle;"> >= forzada el <font style="font-size:120%;color:#000;" title="Fecha Firma Digital Tasador">'+format(@fecha_firma_digital,'dd/MM/yyyy HH:mm:ss')+'</font></span>'
                     when @motivo_no_autorizado is not null then                        '<span style="display:inline-block;width:auto;color:#700;font-size:100%;vertical-align:middle;"> >= Firma Digital Rechazada el <font style="font-size:120%;color:#000;" title="Rechazo Firma Digital Tasador">'+isnull(format(@fecha_firma_digital,'dd/MM/yyyy HH:mm:ss'),'')+'</font></span>'
                     else ''
                end
