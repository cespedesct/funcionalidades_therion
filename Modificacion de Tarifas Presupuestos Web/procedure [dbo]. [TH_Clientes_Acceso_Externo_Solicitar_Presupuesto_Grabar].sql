...
begin try
      ...
      */
      ...
      declare @fichero_dni varchar(100)
      if isnull(@tabla_asociada,'')!=''
         begin
            select top 1 @fichero_dni=convert(varchar(100),f.fileId) 
      ...
      declare @fichero_justificante varchar(100)
      if isnull(@tabla_asociada_justificante,'')!=''
         begin
            select top 1 @fichero_justificante=convert(varchar(100),f.fileId) 
            from SISTEMA_ficheros f (nolock) 
      ...
      declare @fichero_notasimple varchar(100)
      if isnull(@tabla_asociada_notasimple,'')!=''
         begin
            select top 1 @fichero_notasimple=convert(varchar(100),f.fileId) 
            from SISTEMA_ficheros f (nolock) 
      ...
      declare @fichero_transferencia varchar(100)
      if isnull(@tabla_asociada_transferencia,'')!=''
         begin
            select top 1 @fichero_transferencia=convert(varchar(100),f.fileId) 
            from SISTEMA_ficheros f (nolock) 
            ...
            if 1=1   -- Llave de Grabacion
               begin
                   ...
                   */
                   ...
                   if exists (select * from TH_Entidades e where e.codentid=@codentid and e.fk_usuario_comercial is null)  -- Tiene un comercial asignado => no se genera documento de presupuesto
                      begin
                   ...
                   if exists (select * from TH_Entidades e where e.codentid=@codentid and e.fk_usuario_comercial is null)  -- Tiene un comercial asignado => no se genera email
                      begin
                         -------------------------------------
