...
begin try
     ...
     set @html+=
      ...
      set @html+=
          ...
          +' td.etiqueta, td.datoencargo, td.aportacion {font-family:"Open Sans","Helvetica Neue",helvetica,arial,sans-serif;color:black;font-size:0.8vw;padding-right:0.5vw;text-align:left;vertical-align:middle;border-bottom:solid 1px transparent;border-right:solid 4px #92575b}'
     ...
     set @html+='input[type="radio"]  {
      ...
      set @html+=
          ...
          +' td.eticomp      {font-family:"Open Sans","Helvetica Neue",helvetica,arial,sans-serif;color:black;font-size:1vw;font-weight:bold;vertical-align:middle;border-bottom:solid 0px #aaa;padding-top:10px}'
...
--
   ...
   --    +'<tr><td class="eticomp_2" style="padding-left:2vw;display:'+@ver_publico+'"><u>Persona de Contacto para la gestión del trabajo solicitado</u></td></tr>'+char(10)
       ...
       declare @css_div_ficheros varchar(max)='vertical-align:top;display:inline-block;overflow-y:auto;font-family:Roboto Condensed;font-size:0.65vw;width:13vw;border:solid 1px transparent;height:auto'
       declare @css_pg_ficheros  varchar(max)='overflow-y:auto;font-family:Roboto Condensed;font-size:0.55vw;width:12vw;border:solid 1px #eee;height:3vh;'
                     ...
                     +'<input type ="file"'
                           ...
                           +' title="Seleccione el fichero correspondiente al Justificante de Transferencia" '       -- CAMPO FICHERO siendo GUID el identificador a grabar en tabla_asociada
									                  +' onchange="Subeficheros(this, document.getElementById(''PGFiles_transferencia''));" '   -- EVENTO PARA SUBIR SIENDO primer paramámetro THIS O EL CAMPO FICHERO Y SEGUNDO PARAMETRO EL DIV DONDE SE VAN A MOSTRAR LAS EVOLUCIONES
                                                                                                                     -- Genera un registro -> select f.fileName, f.tabla_asociada, f.codigo_tabla_asociada from SISTEMA_ficheros f (nolock) where f.tabla_asociada='TH_Tasadores_'+@semilla_ficheros+'' and codigo_tabla_asociada=@fk_TH_Tasadores
                           +'/><br>' 
                     +'<div id="PGFiles_transferencia" style="'+@css_pg_ficheros+';width:21vw;"></div>' -- DIV DONDE VAN LOS NOMBRES DE FICHEROS Y LA PROGRESS BAR
                 +'</section>'
              +'</div>' 
              -----------------------------------
       +'</tr>'        
       ...
       -----------------------------------
	      set @html+=
       +'<tr style="height:auto;display:'+@ver_publico+'">
             <td class="datoencargo" style="text-align:left;padding-left:0px">'
              +'<div id="sectionimportemanual" style="display:inline-block;width:52vw;border:solid 0px #000;vertical-align:bottom;padding:0px>'
              ...
              +'</div>'
              ------------------------------------------------
              +'<div id="validacionjustificante" style="display:inline-block;width:22vw;border:solid 0px #000 ;vertical-align:top;text-align:left;vertical-align:bottom;">'
                     +'<p style="margin:8px;display:inline-block;text-align:right;width:21vw;padding-right:0px">'
                          +'<a class="btn primary"'
